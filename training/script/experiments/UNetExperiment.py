...
class UNetExperiment:
    ...
    def __init__(self, config, split, dataset):
        ...
        self.train_loader = DataLoader(SlicesDataset(dataset[split["train"]]),
                batch_size=config.batch_size, shuffle=True, num_workers=0)#, collate_fn=None)
        ...
        # Do we have CUDA/Metal available?
        if torch.backends.mps.is_available():
            self.device = torch.device("mps")
        elif torch.cuda.is_available():
            self.device = torch.device("cuda")
        else:      
            print("WARNING: No Metal Apple or CUDA device is found. This may take significantly longer!")        
    ...
    def run_test(self):
        ...
        for i, x in enumerate(self.test_data):
            ...
            print(f"{x['filename']} Sensitivity {sens:.4f}. {100*(i+1)/len(self.test_data):.2f}% complete")
            print(f"{x['filename']} Specificity {spec:.4f}. {100*(i+1)/len(self.test_data):.2f}% complete")
        ...
        out_dict["overall"] = {
            ...
            "max_dice": np.max(dc_list),
            "max_jaccard": np.max(jc_list),
            "max_sensitivity": np.max(sens_list),
            "max_specificity": np.max(spec_list),
            "min_dice": np.min(dc_list),
            "min_jaccard": np.min(jc_list),
